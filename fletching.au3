...
While (1)
	...
	if $timerr == 1 Then
		$length = InputBox("HOW LONG TO RUN", "How many minutes to run? *TOOLBELT ORDER*, 1=File, 2=Mallet, 3=Carving Knife, 4=Log, 5=Pelt *BIND* I Improve/Finish and R repair *Z Starts*", 30)
	...
	if $running == 1 Then
		...
		Local $file = 1
		While $file = 1
			$file = _ImageSearchArea("image/file.png",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
			If $file = 1 Then
				File()
				Repair()
				checktimer()
		...
		Local $knife = 1
		While $knife = 1
			$knife = _ImageSearchArea("image/carving_knife.png",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
			If $knife = 1 Then
				Knife()
				Repair()
				checktimer()
		...
		Local $log = 1
		While $log = 1
			$log = _ImageSearchArea("image/log.png",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
			If $log = 1 Then
				WoodLog()
				Repair()
				checktimer()
		...
		Local $pelt = 1
		While $pelt = 1
			$pelt = _ImageSearchArea("image/pelt.png",1,$ulsearch[0],$ulsearch[1],$lrsearch[0],$lrsearch[1],$x1,$y1,50)
			If $pelt = 1 Then
				Pelt()
				Repair()
				checktimer()
